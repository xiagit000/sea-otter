...
public class FtpStarrocksMigrator implements DataMigrator {

    private final FtpConnector source;
    private final StarrocksConnector sink;
    private final Logger logger = LoggerFactory.getLogger(FtpStarrocksMigrator.class);

    public FtpStarrocksMigrator(FtpConnector source, StarrocksConnector sink) {
        this.source = source;
        this.sink = sink;
    }
    ...
    public void migrate() {
        ...
        String createTableSQL = StarRocksUtils.generateTableCreateSql(statement, sink.getTable(), null);
        try (Connection sinkConnection =  getSinkConnection();
             Statement sinkStatement = sinkConnection.createStatement()) {
            logger.info("FTP create table sql: {}", createTableSQL);
